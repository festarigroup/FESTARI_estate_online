...
class ArtisanProfileViewSet(viewsets.ModelViewSet):
    ...
    def _verify_owner(self, obj):
        if obj.user != self.request.user and not self._is_admin():
            self.permission_denied(self.request)

    def perform_create(self, serializer):
        user = self.request.user
        # Check if user has artisan subscription
        subscription = getattr(user, "subscription", None)
        if not (subscription and subscription.is_active and subscription.plan.is_artisan_plan):
            raise serializers.ValidationError("Artisan subscription required to create profile")
    ...
    def upload_media(self, request, pk=None):
        ...
        # For artisans, use artisan plan limits or default
        user = request.user
        subscription = getattr(user, "subscription", None)
        if subscription and subscription.is_active and subscription.plan.is_artisan_plan:
            ...
            # Default limits for artisans without subscription
            current_images = sum(1 for url in (obj.media_urls or []) if url.lower().endswith(IMAGE_EXTENSIONS))
            current_videos = sum(1 for url in (obj.media_urls or []) if url.lower().endswith(VIDEO_EXTENSIONS))
        ...
        upload_result = upload_media_file(media_file, "artisans", str(obj.id))
        ...
        return api_response(
            True,
            "Artisan media uploaded successfully.",
            status.HTTP_201_CREATED,
            data={"url": upload_result["public_url"], "path": upload_result["path"]},
        )
    ...
    def delete_media(self, request, pk=None):
        ...
        if media_url not in (obj.media_urls or []):
            return api_response(False, "Media URL does not belong to this artisan.", status.HTTP_400_BAD_REQUEST)
