...
class PropertyViewSet(viewsets.ModelViewSet):
    ...
    def _verify_owner(self, obj):
        if obj.owner != self.request.user and not self._is_admin():
            self.permission_denied(self.request)

    def _save_media_url(self, obj, public_url, old_url=None):
        media_urls = obj.media_urls or []
        if old_url and old_url in media_urls:
            media_urls.remove(old_url)
        if public_url not in media_urls:
    ...
    def perform_create(self, serializer):
        ...
        if subscription and subscription.is_active:
            ...
            current_properties = Property.objects.filter(owner=user).count()
            if current_properties >= plan.max_properties:
                raise serializers.ValidationError(f"Property limit reached ({plan.max_properties})")
        else:
            # Default limit for non-subscribed users
            current_properties = Property.objects.filter(owner=user).count()
            if current_properties >= DEFAULT_PROPERTY_LIMIT:  # Default
                raise serializers.ValidationError(f"Property limit reached ({DEFAULT_PROPERTY_LIMIT})")
    ...
    def upload_media(self, request, pk=None):
        ...
        upload_result = upload_media_file(media_file, "properties", str(obj.id))
        if not upload_result.get("success"):
            return api_response(False, upload_result.get("error", "Upload failed."), status.HTTP_400_BAD_REQUEST)

        self._save_media_url(obj, upload_result["public_url"], old_url=old_url)
        if old_url:
            try:
    ...
    def delete_media(self, request, pk=None):
        ...
        if media_url not in (obj.media_urls or []):
            return api_response(False, "Media URL does not belong to this property.", status.HTTP_400_BAD_REQUEST)
        ...
        obj.save(update_fields=["media_urls"])
        return api_response(True, "Property media deleted.", status.HTTP_200_OK, data={"url": media_url})
    ...
    def feature(self, request, pk=None):
        ...
        user = request.user
        subscription = getattr(user, "subscription", None)
        if not (subscription and subscription.is_active and subscription.plan.can_feature_properties):
            return api_response(False, "Subscription does not allow featured properties", status.HTTP_403_FORBIDDEN)

        obj.is_featured = True
        obj.save(update_fields=["is_featured"])
        return api_response(True, "Property featured successfully", status.HTTP_200_OK, data=PropertySerializer(obj).data)
