...
export function MobileBottomNav({ activeKey = "feed", activeChildKey = "home" }: MobileBottomNavProps) {
  ...
  return (
    <nav
      ...
      aria-hidden={!visible}
      className={cn(
        "no-scrollbar fixed bottom-4 left-1/2 z-40 flex w-[calc(100%-32px)] -translate-x-1/2 items-center justify-between overflow-x-auto rounded-full border border-white/40 bg-white/60 p-5 shadow-[0px_4px_10px_rgba(0,0,0,0.15)] backdrop-blur-xl backdrop-saturate-150 transition-all duration-300 ease-out lg:hidden",
