...
export default function PropertyQuickStats({ property }: { property: Property }) {
  const stats = [
    { icon: <BedIcon />, value: property.beds, label: "Bedrooms" },
    { icon: <BathIcon />, value: property.baths, label: "Bathrooms" },
    { icon: <SqftIcon />, value: property.sqft.toLocaleString(), label: "Sq Ft" },
    { icon: <ParkingIcon />, value: property.parking, label: "Parking" },
  ];

  return (
    <div className="grid grid-cols-2 gap-6 rounded-[24px] border border-[rgba(89,112,97,0.2)] bg-white p-8 sm:grid-cols-4">
      {stats.map((stat) => (
