...
const CATEGORIES = ["Hotels", "Resorts", "Boutique Stays"];

const PRICE_RANGES = ["Any price", "Under GHS 900", "GHS 900 – 1,300", "GHS 1,300 – 1,800", "GHS 1,800+"];
  ...
  return (
    <section className="relative flex min-h-[820px] w-full flex-col pb-24">
      <div className="absolute inset-0 overflow-hidden">
        <Image src="/hotel/hotel-hero.jpg" alt="Infinity pool overlooking the ocean at dusk" fill priority className="object-cover" />
      ...
      <motion.div {...fadeUp(0.3)} className="relative z-10 mx-5 md:mx-16">
        <div className="rounded-[32px] border border-white/20 bg-[rgba(50,50,50,0.7)] p-2.5">
          <p className="px-6 py-3 text-2xl font-medium text-white">Find your perfect stay</p>
          ...
          <div className="flex flex-col items-stretch gap-4 p-4 md:flex-row md:items-center">
            ...
            <label className="flex flex-1 flex-col gap-1 border-white/20 px-4 py-1 text-left md:border-r">
              <span className="text-xs font-medium uppercase tracking-[1.4px] text-white">Check-in date</span>
              <input
                type="date"
                value={checkIn}
                onChange={(e) => setCheckIn(e.target.value)}
                className="bg-transparent text-base text-white placeholder:text-[#afafaf] focus:outline-none [color-scheme:dark]"
              />
            </label>
            ...
            <label className="flex flex-1 flex-col gap-1 px-4 py-1 text-left">
              <span className="text-xs font-medium uppercase tracking-[1.4px] text-white">Guests</span>
              <HeroSelect value={guests} placeholder="Add guests" options={GUEST_COUNTS} onChange={setGuests} />
            </label>
