...
export default function SignupPage() {
  ...
  const [errors, setErrors] = useState<{ fullName?: string; emailOrPhone?: string; password?: string }>({});
  ...
  const handleNext = () => {
    if (step === 1) {
      ...
      if (err) { setErrors({ fullName: err }); return; }
    }
    if (step === 2) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const res = await mockRegister(fullName, emailOrPhone, password);
      if (res.success) {
        toast.success("Account created! Welcome to Festari Estates.");
  ...
  const stepCanAdvance =
    (step === 1 && fullName.trim().split(" ").length >= 2) ||
    (step === 2 && emailOrPhone.trim().length > 0);

  return (
    <div className="bg-white rounded-lg shadow-sm py-8 px-30 w-full">
      <div className="space-y-6">
        <div className="text-center space-y-1">
          <h1 className="text-3xl font-bold text-gray-900">HI, USER</h1>
          <p className="text-sm text-gray-600">Create an Account with Us now</p>
        </div>
          ...
          {step === 1 && (
            <div>
              ...
              <input
                ...
                className={`block w-62.5 px-3 py-2 border focus:outline-none focus:ring-[#BE4D00] focus:border-[#BE4D00] text-sm ${errors.fullName ? "border-red-500" : "border-black"}`}
              ...
              {errors.fullName ? (
                ...
                <p className="mt-1 text-xs text-gray-500">Preferably First Name, Middle, Last Name in that order</p>
              )}
            </div>
          )}
          ...
          {step === 2 && (
            <div>
              ...
              {errors.emailOrPhone ? (
                ...
                <p className="mt-1 text-xs text-gray-500">A 4-digit code will be sent to this account to verify it</p>
              )}
            </div>
          )}
          ...
          {step === 3 && (
            <div className="space-y-2">
              ...
              {errors.password ? (
                ...
                <p className="text-xs text-gray-600">At least 8 characters with numbers and symbols</p>
              )}
          ...
          <div className="flex justify-between gap-4">
            ...
            {step < 3 ? (
              <button
                type="button"
                onClick={handleNext}
                disabled={!stepCanAdvance || loading}
                className={`w-20.5 py-2 px-4 text-sm font-medium focus:outline-none ${stepCanAdvance ? "bg-[#BE4D00] text-white hover:bg-orange-700" : "bg-gray-200 text-gray-500 cursor-not-allowed"}`}
              >
                Next
              </button>
            ) : (
              ...
              >
                {loading ? <><Spinner /> Creating…</> : "Sign Up"}
              </button>
            )}
          </div>
        </form>
        ...
        <p className="text-center text-sm text-gray-600">
          ...
          <Link href="/login" className="font-medium text-[#BE4D00] hover:text-orange-700">
            Login for free
          </Link>
        </p>
      </div>
